...
const Page = () => {
    ...
    async function fetchData() {
      try {
        const { data } = await axios.get(
          `${baseurl}/english/user/findbyid/${pram.questionid}`
        );
  ...
  return (
    <div className="bg-gray-100 py-2 min-h-screen">
        ...
        {
            loader ?  <div className="flex justify-center items-center md:w-2/3 mx-auto p-10 rounded-2xl border-2 h-[80vh]"><Image className="w-40 h-40" src={waiting} alt="Loading"/></div> :  <div
            className={`md:w-2/3 mx-auto bg-white p-10 rounded-2xl border-2`}
          >
